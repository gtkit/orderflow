...
func TestDefaultGenerateOrderNo_ExportedHelper(t *testing.T) {
	const N = 100
	seen := make(map[string]struct{}, N)

	no := DefaultGenerateOrderNo(1001)
	if len(no) != 30 {
		t.Fatalf("expected 30 chars, got %d: %q", len(no), no)
	}
	for i, r := range no[:20] {
		if r < '0' || r > '9' {
			t.Fatalf("timestamp pos %d not digit: %q", i, no)
		}
	}
	seen[no] = struct{}{}
	...
	for range N - 1 {
		...
		if _, dup := seen[no]; dup {
			t.Fatalf("duplicate order no generated by exported helper: %q", no)
		}
		seen[no] = struct{}{}
	}
}
