...
func (f *DeliveryFallback[O]) Run(ctx context.Context) {
	f.logger.InfoContext(ctx, "orderflow: delivery fallback scanner started")
	defer f.logger.InfoContext(ctx, "orderflow: delivery fallback scanner stopped")
...
func (f *DeliveryFallback[O]) scan(ctx context.Context) {
	...
	defer func() {
		if r := recover(); r != nil {
			f.logger.ErrorContext(ctx, "orderflow: panic in delivery fallback scan recovered",
				slog.Any("panic", r),
			)
		}
	}()
	...
	if err != nil {
		f.logger.ErrorContext(ctx, "orderflow: delivery fallback scan failed",
			slog.Any("error", err),
		)
		return
	}
	...
	f.logger.InfoContext(ctx, "orderflow: delivery fallback found paid-undelivered",
	...
	for _, orderNo := range orderNos {
		...
		if err := f.engine.ReconcilePaid(taskCtx, orderNo); err != nil {
			f.logger.ErrorContext(ctx, "orderflow: delivery fallback reconcile failed",
